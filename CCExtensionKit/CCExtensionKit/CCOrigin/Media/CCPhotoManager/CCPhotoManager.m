...
NSString * cc_photo_manger_image_key = @"image";
NSString * cc_photo_manger_info_key = @"info";
NSString * cc_photo_manger_video_key = @"video";
NSString * cc_photo_manger_unknow_key = @"unknow";

NSString * cc_photo_manger_type_key = @"type" ;
NSString * cc_photo_manger_asset_key = @"asset" ;
...
- (NSMutableArray <NSDictionary *> *) cc_get_images_page : (NSInteger) i_page {
    ...
    [array_temp enumerateObjectsUsingBlock:^(PHAsset * _Nonnull obj,
        ...
        if (obj.mediaType == PHAssetMediaTypeImage) {
            s_type = cc_photo_manger_image_key;
        }
        else if (obj.mediaType == PHAssetMediaTypeVideo){
            s_type = cc_photo_manger_video_key;
        }
        else s_type = cc_photo_manger_unknow_key;
        
        NSMutableDictionary *dict_info = [NSMutableDictionary dictionary];
        [dict_info setValue:s_type forKey:cc_photo_manger_type_key];
        [dict_info setValue:obj forKey:cc_photo_manger_asset_key];
        ...
        [[PHCachingImageManager defaultManager] requestImageForAsset:obj
               ...
               if (image) {
                   [dict_info setValue:image forKey:cc_photo_manger_image_key];
                   [dict_info setValue:info forKey:cc_photo_manger_info_key];
               } else {
                   is_need_added = false;
               }
           }];
...
- (void) cc_get_image_data : (PHAsset *) asset
                  complete : (void(^)(NSData *image_data ,
                                      BOOL is_can_be_used)) cc_complete_block {
    [[PHCachingImageManager defaultManager] requestImageDataForAsset:asset
                                                       ...
                                                       resultHandler:^(NSData * _Nullable imageData,
                                                                       ...
                                                                       NSDictionary * _Nullable info) {
           if (imageData) {
               UIImage *t_image = [UIImage imageWithData:imageData].cc_fix_orientation;
               NSData *data_new_image = UIImageJPEGRepresentation(t_image, .5f);
               if (data_new_image) {
                   
                   if ([NSThread isMainThread]) {
                       if (cc_complete_block) cc_complete_block(data_new_image , YES);
                   }
                   else dispatch_sync(dispatch_get_main_queue(), ^{
                       if (cc_complete_block) cc_complete_block(data_new_image , YES);
                   });
               }
               else if (cc_complete_block) cc_complete_block(nil , false);
           }
           else if (cc_complete_block) cc_complete_block(nil , false);
       }];
}
...
- (void) cc_get_video_url :  (PHAsset *) asset
    ...
    if (asset.mediaType != PHAssetMediaTypeVideo) {
        if (cc_complete_block) cc_complete_block(nil , false , YES);
        return;
    }
    ...
    [manager requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
        ...
        if (cc_duration_block) {
            NSString *s_time = [NSString stringWithFormat:@"%02ld:%02ld"
                                ,(NSUInteger)(f_time_seconds / 60)
                                ,((NSUInteger)f_time_seconds % 60)];
            cc_duration_block(s_time , f_time_seconds);
        }
        
        if (cc_complete_block) {
            BOOL is_off_limit_times = f_time_seconds > 60.f;
            cc_complete_block(url_asset.URL, YES , is_off_limit_times);
        }
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    if (self.delegate_t
        && [self.delegate_t respondsToSelector:@selector(cc_photo_manager:change:)]) {
        [self.delegate_t cc_photo_manager:self change:changeInstance];
    }
}
...
- (NSMutableArray<PHAsset *> *)array_assests {
    ...
    _array_assests = [self cc_get_all_asset_in_album:self.is_ascending];
