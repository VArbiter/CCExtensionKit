...
- (void) cc_complete_transaction : (SKPaymentTransaction *) transaction
                          status : (CCIAPStatus) status ;

- (void) cc_upload_receipt : (SKPaymentTransaction *) transaction ;
...
- (void) cc_purchase : (NSString *) s_product_id {
    [self cc_purchase:s_product_id result:nil];
}
- (void) cc_purchase : (NSString *) s_product_id
              result : (void(^)(CCIAPManager *manager , id result , NSError *error)) cc_result_block {
    if (cc_result_block) self.cc_result_block = [cc_result_block copy];
    ...
    if ([SKPaymentQueue canMakePayments]) { // user allowed to purchase // 用户允许支付
        ...
        if (self.cc_result_block) self.cc_result_block(self, nil, e);
        if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:error:)]) {
            [self.delegate cc_IAP_manager:self error:e];
        }
    }
}
...
- (void)requestDidFinish:(SKRequest *)request{
    if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:request_did_finish:)]) {
        [self.delegate cc_IAP_manager:self request_did_finish:request];
    }
}
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error{
    if (self.cc_result_block) self.cc_result_block(self, nil, error);
    if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:error:)]) {
        [self.delegate cc_IAP_manager:self error:error];
    }
}
    ...
    for (SKPaymentTransaction *transaction in transactions) {
        ...
        switch (transaction.transactionState)
        {
            ...
            case SKPaymentTransactionStatePurchased:{
                ...
                [self cc_complete_transaction:transaction
...
#endif
    if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:payment_queue:removed:)]) {
        [self.delegate cc_IAP_manager:self
                        payment_queue:queue
...
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error {
    if (self.delegate
        && [self.delegate respondsToSelector:@selector(cc_IAP_manager:payment_queue:restore_completed_fail:)]) {
        [self.delegate cc_IAP_manager:self payment_queue:queue restore_completed_fail:error];
    }
}

- (void)paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue {
    if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:restore_completed:)]) {
        [self.delegate cc_IAP_manager:self restore_completed:queue];
    }
}
    ...
    if (transaction.error.code != SKErrorPaymentCancelled){
        switch (status) {
            ...
            case CCIAPStatus_Restore_Success:{
                [self cc_upload_receipt:transaction];
            }break;
            case CCIAPStatus_Purchase_Fail:
            case CCIAPStatus_Restore_Fail:
            default:{
                if(self.cc_result_block) self.cc_result_block(self, nil, transaction.error) ;
                if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:error:)]) {
                    [self.delegate cc_IAP_manager:self error:transaction.error];
                }
...
- (void) cc_upload_receipt : (SKPaymentTransaction *) transaction {
    ...
    if (self.cc_upload_receipt_block) {
        if (self.cc_upload_receipt_block(self, url_receipt, d_params)) {
            [SKPaymentQueue.defaultQueue finishTransaction:transaction];
        };
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(cc_IAP_manager:original_url:final_result:)]) {
        if ([self.delegate cc_IAP_manager:self original_url:url_receipt final_result:d_params]) {
            [SKPaymentQueue.defaultQueue finishTransaction:transaction];
        }
    }
}
