...
void CC_DISPATCH_ASYNC_M(void (^mq_action_block)(void)) {
    CC_DISPATCH_ASYNC(dispatch_get_main_queue(), mq_action_block);
}
void CC_DISPATCH_SYNC_M(void (^mq_action_block)(void)) {
    CC_DISPATCH_SYNC(dispatch_get_main_queue(), mq_action_block);
}
void CC_DISPATCH_ASYNC(dispatch_queue_t queue , void (^mq_action_block)(void)) {
    if (!queue) return ;
    dispatch_async(queue ? queue : CC_MAIN_QUEUE(), mq_action_block);
}
void CC_DISPATCH_SYNC(dispatch_queue_t queue , void (^mq_action_block)(void)) {
    if (pthread_main_np() != 0) {
        if (mq_action_block) mq_action_block();
    }
    else dispatch_sync(queue ? queue : CC_MAIN_QUEUE(), mq_action_block);
}

void CC_DISPATCH_BARRIER_ASYNC(dispatch_queue_t queue , void (^mq_action_block)(void)){
    dispatch_barrier_async(queue ? queue : CC_MAIN_QUEUE(), mq_action_block);
}

void CC_DISPATCH_APPLY_FOR(size_t count , dispatch_queue_t queue , void (^mq_time_block)(size_t t)) {
    dispatch_apply(count, queue ? queue : CC_MAIN_QUEUE(), mq_time_block);
}
